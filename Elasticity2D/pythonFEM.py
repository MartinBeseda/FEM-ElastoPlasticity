...
def elasticity_fem(element_type: LagrangeElementType = LagrangeElementType.P1,
    ...
    # x, res, rank, s = np.linalg.lstsq(stiff_mat.A, f[Q_flat].A.transpose())
    # print(f'result: {x}')
    # print(f'a shape: {s.shape}, a rank: {rank}, singular values: {s}')
    # u.transpose()[mesh['Q'].transpose()] = x.flatten(order='F')
